...
import { packCursor, unpackCursor } from "@/lib/search/cursor";
import { termsSchema } from "@/lib/search/contracts";
export const runtime = "nodejs";
export const maxDuration = 120;
const schema = z.object({ scope: scopeSchema, terms: termsSchema, cursor: z.string().max(1_500_000).nullable() }).strict();
export async function POST(request: NextRequest) {
  try {
    ...
    const state = value.cursor ? unpackCursor(value.cursor, config.key, session.accessToken) : await startSearch(session.accessToken, value.scope, value.terms, session.expiresAt, fetch, signal);
