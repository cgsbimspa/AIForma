...
export async function POST(request: NextRequest) {
  try {
    ...
    const state = value.cursor ? unpackCursor(value.cursor, config.key, session.accessToken) : await startSearch(session.accessToken, value.scope, value.terms, session.expiresAt, fetch, signal, value.stage);
