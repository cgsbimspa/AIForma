...
export default function Home() {
  return <div className="page-content">
    ...
    <div className="module-grid">{modules.map((module) => <Link className={`module-card ${module.slug === "incidencias" ? "transversal-card" : ""}`} href={`/${module.slug}`} key={module.slug}><div className="card-top"><span className="module-icon"><module.icon size={23} strokeWidth={1.7}/></span><span className="phase-label">{module.phase}</span><ArrowUpRight className="card-arrow" size={19}/></div><h3>{module.name}</h3><p>{module.description}</p><div className="card-bottom"><span className="pending-mark"/>{module.slug === "asistente" ? "Explorador Forma + Asistente IA" : module.slug === "cubicaciones" ? "Configuración y trazabilidad" : "Pendiente de implementación"}</div></Link>)}</div></section>
