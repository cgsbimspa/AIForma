...
import { liveCalculationSchema, type CalculationEvent } from "@/lib/quantities/live";

export function QuantityViewer({ project, source, highlightedElementIds=[], filteredElementIds=null, onSelectElements, classificationFilter, calculationRequest=0, onCalculation }: { project: QuantityProject; source: QuantitySource | null; highlightedElementIds?:string[]; filteredElementIds?:string[]|null; onSelectElements?:(ids:string[])=>void; classificationFilter?: {specialty:string; subspecialty:string}; calculationRequest?:number; onCalculation?:(event:CalculationEvent)=>void }) {
  ...
  }, [source?.view?.id, source?.version.modelId,onSelectElements,selection,calculationRequest,onCalculation]);
  useEffect(()=>{
  ...
  function visibility(action:'isolate'|'hide'|'showAll') {
