...
export const ENGINE='view-quantities-v2.5';
...
export function filterRecords(records,filter){const selected=filter.selection==null?null:new Set(filter.selection);return records.filter(e=>matchesFilter(e,filter,selected));}
export function quantityFacets(records,filter){const unique=a=>[...new Set(a)].sort((a,b)=>a.localeCompare(b,'es',{numeric:true}));return {specialties:unique(filterRecords(records,{...filter,specialty:[]}).map(e=>filterValues(e).specialty)),categories:unique(filterRecords(records,{...filter,category:[]}).map(e=>filterValues(e).category)),floors:unique(filterRecords(records,{...filter,floor:[]}).map(e=>filterValues(e).floor))};}
export function presentQuantities(data,filter){
 const records=filterRecords(data.records,filter),coverage=Object.fromEntries(['concrete','formwork','rebar'].map(m=>[m,metricCoverage(records,m)]));
