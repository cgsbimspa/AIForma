...
export async function answerDocuments(evidence: DocumentEvidence, question: string, mode: DocumentMode, config: { key: string; model: string }, fetcher: typeof fetch = fetch, signal?: AbortSignal): Promise<DocumentAnswer> {
  ...
  const instructions = [
    ...(mode === "summary" ? ["Entrega un resumen general legible, no una lista de citas inconexas. Empieza por una síntesis de qué trata el contenido leído y su propósito, sólo si están respaldados; desarrolla después los temas principales en pocos párrafos con títulos breves. Incluye conclusiones, condiciones, excepciones o pendientes únicamente cuando el texto los declare. Adapta la extensión al contenido: no rellenes apartados ni fuerces conclusiones en documentos breves, correos o planos. No deduzcas el tema ni el propósito sólo del nombre del archivo. Si hay varios documentos, distingue sus aportes y no atribuyas a uno lo que sólo dice otro. No presentes el resumen como completo cuando partial=true. Cada párrafo necesita sus propias citas y todas sus afirmaciones deben quedar respaldadas."] : []),
  ...
  if (streetLabels) {
    // Text extraction has no geometric relation engine. Present only the bound
    // labels, never the model's inferred adjacency, access or future extensions.
    blocks = blocks.map(block => ({...block, label:"", text:`En el plano puedo leer: ${[...new Set(block.citations.map(c => canonical(c.quote)))].map(q => `«${q}»`).join("; ")}.`}));
    base.warnings = [...base.warnings, "Estos rótulos aparecen en el plano. Su lectura por sí sola no permite determinar qué calles colindan con el predio ni cuáles dan acceso a él."];
  }
