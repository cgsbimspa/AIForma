...
export function isDocumentQuestion(text: string) {
  ...
  return /^(?:que|cuales?|cuanto[sa]?|como|por que|a que|en que|indicame|explica(?:me)?|dime)\b/.test(value) || /\b(?:que dice|que indica|que muestra|que aparece|que contiene|segun el documento)\b/.test(value);
}
