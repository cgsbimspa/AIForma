...
export const cookieName = (config: Config, kind: "session" | "attempt" | "renewal") => `${config.secure ? "__Host-" : ""}ai-forma-aps-${kind}`;
export function setCookie(response: NextResponse, config: Config, kind: "session" | "attempt" | "renewal", value: string, maxAge: number) {
