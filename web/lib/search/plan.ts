...
export async function planSearch(messages: { role: string; content: string }[], key: string, model: string, signal?: AbortSignal) {
  ...
  try {
    ...
    plan.terms = termsSchema.parse([...new Map(groups.map(group => [[...group].sort().join("|"), group])).values()]);
    return plan;
  }
