...
export async function parseDocument(bytes: Buffer, name: string, signal?: AbortSignal, options: { startPage?: number } = {}): Promise<ParsedDocument> {
  ...
  try {
    ...
    return await new Promise((accept, reject) => {
      const child = spawn(process.execPath, ["--max-old-space-size=512", resolve("worker/document-worker.mjs"), file, name, String(startPage)], { windowsHide: true, stdio: ["ignore", "pipe", "ignore"], env: { NODE_ENV: process.env.NODE_ENV, PATH: process.env.PATH, SystemRoot: process.env.SystemRoot } });
      let output = "", settled = false;
      const cleanup = () => { clearTimeout(timer); signal?.removeEventListener("abort", abort); };
      const fail = (code: string) => { if (settled) return; settled = true; child.kill(); cleanup(); reject(new DataError(code, 422)); };
      const abort = () => fail("parse_cancelled"), timer = setTimeout(() => fail("parse_timeout"), 75_000);
